...
function normalizeAnswer(answer, optionCount) {
  const values = Array.isArray(answer) ? answer : [answer];

  if (values.length === 1) {
    const compact = String(values[0]);
    const numericValue = Number(compact);

    // 旧题库会将多选答案保存成 123 或 1234，分别代表 A、B、C 与 A、B、C、D。
    if (/^\d{2,}$/.test(compact) && numericValue > optionCount) {
      return compact
        .split("")
        .map(Number)
        .filter(value => value >= 1 && value <= optionCount);
    }
  }
...
async function initialize() {
  try {
    ...
    state.questions = data.questions.map(question => ({
      ...question,
      answer: normalizeAnswer(question.answer, question.options.length)
    }));
